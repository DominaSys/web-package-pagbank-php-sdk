...
it('creates account with connect credentials', function () use ($makeSdkWithHistory): void {
    $history = [];
    ...
    Assert::assertSame('client-id', $history[0]['request']->getHeaderLine('x-client-id'));
    Assert::assertSame('client-secret', $history[0]['request']->getHeaderLine('x-client-secret'));
    Assert::assertSame('https://sandbox.api.pagseguro.com/accounts', (string) $history[0]['request']->getUri());
...
it('reads account with client token', function () use ($makeSdkWithHistory): void {
    $history = [];
    ...
    Assert::assertSame('Bearer bearer-token', $history[0]['request']->getHeaderLine('Authorization'));
    Assert::assertSame('client-token-123', $history[0]['request']->getHeaderLine('x-client-token'));
    Assert::assertSame('https://sandbox.api.pagseguro.com/accounts/ACCO_F74B8490-77C0-4F9C-88EA-E98735B4CB36', (string) $history[0]['request']->getUri());
});

it('maps account api validation errors to domain exception', function (): void {
    $sdk = PagBank::make(
        Configuration::make(
            endpoints: new Endpoints(environment: Environment::Sandbox),
            credentials: new Credentials(
    ...
    expect(fn (): mixed => $sdk->accounts()->createAccount(AccountData::buyer(
