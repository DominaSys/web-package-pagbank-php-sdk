...
it('charge response exposes typed accessors', function (): void {
    ...
    Assert::assertSame('PAID', $response->status());
    Assert::assertSame('2024-01-01T10:00:00-03:00', $response->createdAt());
    Assert::assertSame(150099, $response->amount()?->value());
    Assert::assertSame(150099, $response->amount()?->summary()?->total());
    Assert::assertSame(20000, $response->paymentResponse()?->code());
    Assert::assertSame('SUCESSO', $response->paymentResponse()?->message());
    Assert::assertSame('CREDIT_CARD', $response->paymentMethod()?->type());
    Assert::assertSame('visa', $response->paymentMethod()?->card()?->brand());
    Assert::assertSame('GOOGLE_PAY', $response->paymentMethod()?->tokenData()?->wallet());
    Assert::assertSame('THREEDS', $response->paymentMethod()?->authenticationMethod()?->type());
});
