...
it('create order data converts to api payload', function (): void {
...
it('order response exposes typed accessors', function (): void {
    ...
    Assert::assertSame('ORDE_F87334AC-BB8B-42E2-AA85-8579F70AA328', $response->id());
    Assert::assertSame('ex-00001', $response->referenceId());
    Assert::assertSame('2020-11-21T23:23:22.69-03:00', $response->createdAt());
    Assert::assertCount(1, $response->items());
    Assert::assertInstanceOf(OrderItemResponse::class, $response->items()[0]);
    Assert::assertSame('nome do item', $response->items()[0]->name());
    Assert::assertInstanceOf(OrderCustomerResponse::class, $response->customer());
    Assert::assertSame('Jose da Silva', $response->customer()?->name());
    Assert::assertCount(1, $response->customer()?->phones() ?? []);
    Assert::assertInstanceOf(OrderPhoneResponse::class, $response->customer()?->phones()[0] ?? null);
    Assert::assertInstanceOf(OrderShippingResponse::class, $response->shipping());
    Assert::assertSame('Avenida Brigadeiro Faria Lima', $response->shipping()?->address()?->street());
    Assert::assertCount(1, $response->qrCodes());
    Assert::assertInstanceOf(OrderQrCodeResponse::class, $response->qrCodes()[0]);
    Assert::assertSame('QRCO_1', $response->qrCodes()[0]->id());
    Assert::assertInstanceOf(OrderChargeResponse::class, $response->charges()[0] ?? null);
    Assert::assertSame('PAID', $response->charges()[0]->status());
    Assert::assertInstanceOf(OrderAmountResponse::class, $response->charges()[0]->amount());
    Assert::assertSame(1000, $response->charges()[0]->amount()?->summary()?->paid());
    Assert::assertInstanceOf(OrderPaymentMethodResponse::class, $response->charges()[0]->paymentMethod());
    Assert::assertSame('CREDIT_CARD', $response->charges()[0]->paymentMethod()?->type());
    Assert::assertInstanceOf(OrderCardResponse::class, $response->charges()[0]->paymentMethod()?->card());
    Assert::assertSame('visa', $response->charges()[0]->paymentMethod()?->card()?->brand());
    Assert::assertInstanceOf(OrderPaymentResponse::class, $response->charges()[0]->paymentResponse());
    Assert::assertSame(20000, $response->charges()[0]->paymentResponse()?->code());
    Assert::assertInstanceOf(OrderLinkResponse::class, $response->links()[0] ?? null);
    Assert::assertSame('SELF', $response->links()[0]->rel());
